...
## Influencer Detail Slide-out Panel — Chunk 1 (DONE)
- Clicking a card's arrow now opens a right-side slide-out panel instead of navigating to `/influencers/:id`. The manager peeks at a creator and closes without leaving the database list.
- Extracted every detail tile (Reliability, Performance, Platforms, Portal management, Contact, Commercials, Content fit, Collaborations, Notes) from the InfluencerDetail page into a shared `InfluencerDetailContent.tsx`. The page and the panel both import these tiles — zero logic duplication, no drift. The page keeps its 12-col masonry; the panel stacks them single-column.
- New `InfluencerDetailPanel.tsx`: portal-mounted slide-out (560px desktop, full-screen mobile). Slide-in via rAF + `translate-x` transition; slide-out delays the parent unmount by 300ms for a clean exit. Closes on X / Esc (unless the edit modal is open) / backdrop click. Locks background scroll while open.
- Panel header: avatar, tier badge, status badge, city, English + Arabic name (RTL), and an "X/3 reliability collabs" indicator. Action row with WhatsApp deep-link + Creator portal link. All existing actions — edit (form modal), delete, rotate / pause / blacklist, copy URL — work inside the panel.
- `NotesTile` is now an editable textarea (saves via `useUpdateInfluencer`) shared by both surfaces, replacing the page's old read-only/conditional notes block.
- Two helper utils extracted to keep things DRY + lint-clean: `utils/whatsapp.ts` (`toWaNumber`/`toWaUrl`, now shared by the card and the panel) and `utils/influencer-format.ts` (`getInitials`, `statusBadgeClass`, `portalUrl`, `RELIABILITY_MIN_COLLABS` — moved out of the component file so it only exports components, silencing react-refresh warnings).
- No InfluencerCard edit needed for the open behavior — the arrow already delegated to `onView`; the page just repoints `onView` to open the panel. The `/influencers/:id` route stays as a fallback so any deep links elsewhere keep working.
- Out of scope (Chunk 2 — layout polish): stat-card grid (Followers / Std Rate / Reliability), reliability composite as a circle viz, exact section ordering from the mockup. Out of scope (needs backend): engagement %, est. views, min CPM, last-paid date, "Add to campaign", vanity portal URLs.

## Influencer Detail Slide-out Panel — Chunk 2 (DONE)
- Visual polish pass to match the mockup. Redesigned the shared tiles in `InfluencerDetailContent.tsx` so both the panel and the fallback page get the new look:
  - **Section cards** (`Tile`) now use eyebrow labels + an optional right-side `action` slot (e.g. "Manage", collab count).
  - **Platforms**: icon-circle rows (TikTok obsidian / Instagram gradient / Snapchat yellow) with a PRIMARY tag on the highest-follower linked platform, follower count, external-open for linked, and a "Link" affordance (opens the edit modal) for unlinked.
  - **Reliability**: horizontal progress bars (Post / Tag / On-time) tinted by score, a "Score active" 3-dot header, and the gated <3-collab preview state.
  - **Commercials**: added a derived **Min CPM** row (`std_rate ÷ followers/1000` → e.g. 3.61 SAR/1K) alongside Std rate + Barter.
  - **Content fit**: label-left / chips-right rows for Niches, Languages, and City.
  - **Contact**: Display name, WhatsApp, Primary handle (highest-follower platform), City.
  - **Collaborations**: rate column (entry budget), newest-first sort, and a "View N more" expander past the first 3.
- Panel-specific additions on top of the shared tiles: a 4-up **stat-card row** (Followers + tier / Std rate + barter / Engagement / Views), a reliability **SVG progress ring** in the header (caps at 3 collabs), and a WhatsApp action button that shows the number inline. Section order now follows the mockup (Platforms → Reliability + Commercials 2-up → Content fit → Contact → Portal → Collabs → Notes). Panel surface switched to `bg-warmbg` for the cream backdrop.
- Honest-data calls (no fabrication): Engagement and Views stat cards show **logged totals** from performance logs (the mockup's "6.8% engagement" / "540K est. views per post" rates aren't tracked) and render "—" until a creator has logged performance. **Last Paid** omitted (no payments table). **"Add to campaign"** omitted (no influencer↔campaign linkage). Portal URL stays the real token URL, not a vanity slug.
